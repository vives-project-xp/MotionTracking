...
def draw_mickey_ears(surf, x, y, scale=1.0):
    """Teken Mickey's oortjes"""
    ear_size = int(40 * scale)
    ear_offset = int(35 * scale)
    # Linkeroortje
    pygame.draw.circle(surf, (0, 0, 0), (int(x - ear_offset), int(y - ear_offset)), ear_size)
    # Rechteroortje
    pygame.draw.circle(surf, (0, 0, 0), (int(x + ear_offset), int(y - ear_offset)), ear_size)

def draw_mickey_nose(surf, x, y, scale=1.0, color=(0, 0, 0)):
    """Teken Mickey's neus (groot zwart ovaal)"""
    width = int(25 * scale)
    height = int(20 * scale)
    pygame.draw.ellipse(surf, color, (int(x - width/2), int(y - height/2), width, height))

def draw_mickey_face(surf, x, y, scale=1.0):
    """Teken Mickey's gezicht (hoofd + oortjes)"""
    # Gezicht (grote cirkel)
    face_radius = int(50 * scale)
    pygame.draw.circle(surf, (255, 200, 100), (int(x), int(y)), face_radius)
    pygame.draw.circle(surf, (0, 0, 0), (int(x), int(y)), face_radius, 3)
    
    # Oortjes
    draw_mickey_ears(surf, x, y, scale)
    ...
    # Neus
    draw_mickey_nose(surf, x, y, scale, (0, 0, 0))

def draw_mickey_glove(surf, x, y, size=15):
    """Teken Mickey's handschoen"""
    # Handschoen basis (witte cirkel)
    pygame.draw.circle(surf, (255, 255, 255), (int(x), int(y)), size)
    pygame.draw.circle(surf, (0, 0, 0), (int(x), int(y)), size, 2)
    # Vingers (4 kleine bolletjes)
    ...
    for angle in angles:
        ...
        pygame.draw.circle(surf, (255, 255, 255), (int(fx), int(fy)), int(size * 0.4))
        ...
        for p_idx, person in enumerate(people):
            try:
                ...
                if "nose" in person:
                    ...
                    # Teken Mickey gezicht
                    draw_mickey_face(effect_surface, fx, fy, scale=1.5)
                ...
                for hand_type, hand_idx in [("left_hand", 9), ("right_hand", 10)]:
                    if hand_type in person:
                        ...
                        # Teken Mickey handschoen
                        draw_mickey_glove(effect_surface, hx, hy, size=20)
                        
                        # Wat deeltjes effect rond handschoenen
                        for _ in range(cfg.get("spawn", 5) // 2):
