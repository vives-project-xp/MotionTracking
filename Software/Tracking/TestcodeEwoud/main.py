...
def run_tracker(shared_queue, stop_event, is_calibrated_flag):
    ...
    # --- NIEUWE ARUCO DETECTOR SETUP (OpenCV 4.7+) ---
    ...
    while not stop_event.is_set():
        ...
        if not local_locked:
            ...
            # Gebruik de nieuwe detector methode
            ...
            if ids is not None:
                ...
                # Controleer of alle 4 de hoek-markers aanwezig zijn
                if all(id in points for id in [0, 1, 2, 3]):
                    ...
                    # Hoeken verzamelen: 0=TL, 1=TR, 2=BL, 3=BR
                    # We pakken de buitenste hoekpunten van elke marker voor het vlak
                    tl = points[0][0] 
                    tr = points[1][1] 
                    br = points[3][2] 
                    bl = points[2][3] 
                    ...
                    if elapsed >= REQUIRED_STABLE_TIME:
                        ...
                        # Matrix berekenen voor perspectief correctie
                        src_pts = fixed_pts.reshape(4, 2).astype(np.float32)
                        dst_pts = np.array([[0,0], [1,0], [1,1], [0,1]], dtype=np.float32)
                        transform_matrix = cv2.getPerspectiveTransform(src_pts, dst_pts)
                        
                        # Voor pointPolygonTest moet de array een specifieke shape hebben
                        fixed_pts_poly = fixed_pts.reshape((-1, 1, 2)).astype(np.int32)
        ...
        # --- FASE 2: TRACKING ---
            ...
            # YOLO detectie op lage resolutie voor snelheid
            ...
            for r in results:
                for box in r.boxes:
                    ...
                    # Bepaal het middelpunt (op borsthoogte voor stabiliteit)
                    center_x = int((x1 + x2) / 2)
                    center_y = int(y1 + (y2 - y1) * 0.3)

                    # Check of persoon binnen de geprojecteerde markers staat
                    # We gebruiken de eerder opgeslagen polygoon
...
# --- PROCES 2: VISUALIZER (Main Thread) ---
def run_visualizer(shared_queue, stop_event, is_calibrated_flag):
    ...
    # --- MARKERS LADEN ---
    try:
        m0 = pygame.transform.scale(pygame.image.load("marker0.png"), (MARKER_SIZE, MARKER_SIZE))
        m1 = pygame.transform.scale(pygame.image.load("marker1.png"), (MARKER_SIZE, MARKER_SIZE))
        m2 = pygame.transform.scale(pygame.image.load("marker2.png"), (MARKER_SIZE, MARKER_SIZE))
        m3 = pygame.transform.scale(pygame.image.load("marker3.png"), (MARKER_SIZE, MARKER_SIZE))
    except Exception as e:
        print(f"WAARSCHUWING: Marker plaatjes niet gevonden! Gebruik rode vlakken. {e}")
