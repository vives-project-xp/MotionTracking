...
# --- 2. EFFECTEN ---
EFFECTS = {
    "MAGIC": {"colors": [(150, 50, 255), (100, 200, 255)], "gravity": 0.0, "decay": (4, 10), "size": (3, 8), "spawn": 8, "ring_count": 2, "trail": 40},
    "FIRE": {"colors": [(255, 60, 0), (255, 150, 0), (255, 230, 50)], "gravity": -0.3, "decay": (8, 15), "size": (4, 10), "spawn": 15, "ring_count": 0, "trail": 60},
    "CYBER": {"colors": [(0, 255, 150), (0, 255, 255)], "gravity": 0.0, "decay": (15, 25), "size": (2, 5), "spawn": 20, "ring_count": 3, "trail": 20},
    "GHOST": {"colors": [(200, 200, 255), (255, 255, 255)], "gravity": 0.02, "decay": (2, 5), "size": (2, 4), "spawn": 4, "ring_count": 1, "trail": 10}
}
...
def run_tracker(shared_queue, stop_event, is_calibrated_flag):
    ...
    while not stop_event.is_set():
        ...
        if not local_locked:
            ...
            data = {"body": None, "hands": []}
            for r in results:
                if r.keypoints and len(r.keypoints.xy) > 0:
                    kpts = r.keypoints.xy[0].cpu().numpy()
                    if len(kpts) > 10:
                        nose, l_hand, r_hand = kpts[0], kpts[9], kpts[10]
                        if cv2.pointPolygonTest(fixed_pts.astype(np.int32), (float(nose[0]), float(nose[1])), False) >= 0:
                            ...
                            data["body"] = (float(tp[0]), float(tp[1]))
                            for hand in [l_hand, r_hand]:
                                if hand[0] > 0:
                                    tp_h = cv2.perspectiveTransform(np.array([[[hand[0], hand[1]]]], dtype=np.float32), transform_matrix)[0][0]
                                    data["hands"].append((float(tp_h[0]), float(tp_h[1])))
                        break
            if data["body"] and not shared_queue.full():
                shared_queue.put(data, block=False)
    cap.release()
...
def run_visualizer(shared_queue, stop_event, is_calibrated_flag):
    ...
    body_x, body_y, hand_pts, particles = WIDTH/2, HEIGHT/2, [], []
    ...
    while not stop_event.is_set():
        ...
        if is_calibrated_flag.value == 0:
            screen.fill(WIT)
            screen.blit(marker_imgs[0], (50, 50))
            screen.blit(marker_imgs[1], (WIDTH-250, 50))
            screen.blit(marker_imgs[3], (WIDTH-250, HEIGHT-250))
            screen.blit(marker_imgs[2], (50, HEIGHT-250))
            pygame.display.flip()
            continue

        if frame_count % 20 == 0:
            try:
                r = requests.get(f"http://{VM_IP}:5000/get_config", timeout=0.08)
        ...
        msg = None
        try:
            while not shared_queue.empty(): msg = shared_queue.get_nowait()
        except: pass

        if msg:
            if msg["body"]: body_x, body_y = msg["body"][0]*WIDTH, msg["body"][1]*HEIGHT
            hand_pts = msg["hands"]

        trail.set_alpha(cfg["trail"])
        screen.blit(trail, (0,0))
        scale, v_y = cfg['current_size_val'] / 5.0, body_y - cfg['offset_px']

        for _ in range(cfg["spawn"]):
            ang, r = random.uniform(0, 6.28), random.uniform(30, 70) * scale
            particles.append(Particle(body_x + math.cos(ang)*r, v_y + math.sin(ang)*r, cfg))

        for h in hand_pts:
            hx, hy = h[0]*WIDTH, h[1]*HEIGHT
            for _ in range(3): particles.append(Particle(hx, hy, cfg, is_hand=True))
